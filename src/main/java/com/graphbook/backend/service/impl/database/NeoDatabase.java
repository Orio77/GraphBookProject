...
/**
 * Implementation of the {@link IDatabase} interface using Neo4j as the backend
 * database.
 * This class provides methods to connect to a Neo4j database, save PDF text
 * elements,
...
public class NeoDatabase implements IDatabase {
     ...
     * If the connection fails, an appropriate message is printed, and an exception
    ...
    public void connect() {
        try {
            ...
            // AuthTokens.basic("neo4j", "123456siedem"));
    ...
    /**
     * Saves a list of {@link PDFText} elements to the Neo4j database under the
     * specified label.
     * Each element is saved as a node with the provided label and a unique element
     * ID.
     *
     * @param texts The list of {@link PDFText} elements to be saved. Must not be
     *              null or empty.
     * @param label The label to be assigned to each node. Must not be null or
     *              empty.
        ...
        try (Session session = driver.session()) {
            ...
            String query = parameters.keySet().stream()
                    .map(key -> {
                        ...
                        return String.format("CREATE (n%d:Page:`%s` {text: $%s, elementId: '%s%d'})", idx, safeLabel,
                                key, safeLabel, idx);
                    })
    ...
    /**
     * Creates an edge between two {@link PDFText} nodes with a specified similarity
     * score and page number.
     * The nodes are identified by their text content, and the edge is labeled with
     * the page number.
     *
    ...
    /**
     * Creates edges between nodes based on a provided map of results. Each entry in
     * the map represents
     * a node and its associated edges, with weights representing the edge
     * properties.
     *
     * @param result A map where the key is the node ID and the value is a list of
     *               pairs representing
     *               the other node IDs and their corresponding weights.
     * @param label  The label to be assigned to the nodes.
     * @throws IllegalArgumentException if result is null or empty, or if label is
     *                                  null or empty.
     */
    ...
    /**
     * Creates edges between a concept node and a list of PDF text nodes based on
     * their scores.
     * Each edge is labeled with the concept name and the corresponding score.
     *
     * @param concept       The concept name to be used as the label for the concept
     *                      node.
     * @param conceptScores A list of pairs representing the PDF text node IDs and
     ...
     * Each concept is associated with a list of pairs representing node IDs and
     * their weights.
     *
     * @param chosenConcepts A list of concept names to retrieve nodes and edges
     *                       for.
     * @return A map where the key is the concept name and the value is a list of
     *         pairs representing node IDs and weights.
     */
